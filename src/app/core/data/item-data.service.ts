...
export abstract class BaseItemDataService extends IdentifiableDataService<Item> implements CreateData<Item>, PatchData<Item>, DeleteData<Item> {
  ...
  protected getitemProgressreport = 'findByStartDateAndEndDate';
  ...
  public getMyBookmark(searchMethod: string, options?: FindListOptions, useCachedVersionIfAvailable?: boolean, reRequestOnStale?: boolean, ...linksToFollow: FollowLinkConfig<Item>[]): Observable<RemoteData<PaginatedList<Item>>> {
   ...
   public _getprogressReportByDate(fromdate: string, todate: string, options: FindListOptions = {}, useCachedVersionIfAvailable = false, reRequestOnStale = true, ...linksToFollow: FollowLinkConfig<Item>[]): Observable<RemoteData<PaginatedList<Item>>> {
    options = Object.assign({}, options, {
      searchParams: [new RequestParam('startdate', fromdate), new RequestParam('enddate', todate)]
    });
    return this.searchData.searchBy(this.getitemProgressreport, options, useCachedVersionIfAvailable, reRequestOnStale, ...linksToFollow).pipe(
...
export class ItemDataService extends BaseItemDataService {
  constructor(
   ...
   public getCitetion(itemid:string,options?: FindListOptions, useCachedVersionIfAvailable?: boolean, reRequestOnStale?: boolean, ...linksToFollow: FollowLinkConfig<Item>[]):any {
    let href1 = "";
    ...
    hrefObs.pipe(
        ...
        href = href + "/getCitationData?itemid=" + itemid + "&formate=IEEE";
        href1 = href;
        const request = new GetRequest(this.requestService.generateRequestId(), href);
        this.requestService.send(request);
    ...
    return this.rdbService.buildSingle<Item>(href1);
     
}
public downloadCitetion(itemid:string,selectionType:string,options?: FindListOptions, useCachedVersionIfAvailable?: boolean, reRequestOnStale?: boolean, ...linksToFollow: FollowLinkConfig<Item>[]):any {
  let href1 = "";
  const hrefObs = this.getEndpoint().pipe(
      isNotEmptyOperator(),
      take(1)
...
public getexcleEndpoint(): Observable<string> {
    ...
    // href$.subscribe((data: string) => {
    //   debugger;
    //   url=data + '/report/downloadItemReport';
    //   return url;
    // });
  }
