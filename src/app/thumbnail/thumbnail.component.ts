...
import { defaultCommunityImage, defaultitemcardImage } from '../app-config';
...
export class ThumbnailComponent implements OnChanges {
  ...
  /** * Indicate if the thumbnail is for a community (true) or an item (false, default)
   */
  @Input() fromCommunity?: boolean = false;
  /**
   * Whether the thumbnail is currently loading
   * Start out as true to avoid flashing the alt text while a thumbnail is being loaded.
   */
  isLoading: WritableSignal<boolean> = signal(true);
  image1 = defaultitemcardImage['defaultthumbnailForItem'];
  image2 = defaultCommunityImage['defaultthumbnailForCommunity'];
  constructor(
